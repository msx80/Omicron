...
/** A SpriteBatch clone that with a super hackish hack, avoids bleeding on nearby pixels when scaling/rotating/etc
 * Unfortunately, subclassing won't work since all fields are private
 * @see Batch
 * @author msx80 */
public class NonBleedingSpriteBatch implements Batch {
	 ...
	 * @see NonBleedingSpriteBatch#NonBleedingSpriteBatch(int, ShaderProgram) */
	public NonBleedingSpriteBatch () {
		this(1000, null);
	}

	/** Constructs a SpriteBatch with one buffer and the default shader.
	 * @see NonBleedingSpriteBatch#NonBleedingSpriteBatch(int, ShaderProgram) */
	public NonBleedingSpriteBatch (int size) {
		this(size, null);
	}
	...
	public void draw (Texture texture, float x, float y, float originX, float originY, float width, float height, float scaleX,
		...
		float epsilon = 0.0000001f; // move the corner a bit inside to avoid bleeding on neighbor pixels
		float u = (srcX) * invTexWidth + epsilon;
		float v = ((srcY) + srcHeight) * invTexHeight -epsilon;
		float u2 = ((srcX) + srcWidth) * invTexWidth -epsilon;
		float v2 = (srcY) * invTexHeight +epsilon;
