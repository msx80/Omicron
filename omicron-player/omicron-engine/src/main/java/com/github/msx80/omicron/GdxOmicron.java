...
public final class GdxOmicron extends ApplicationAdapter implements AdvancedSys {
	...
	public GdxOmicron(Cartridge cartridge, HardwareInterface hw, GdxOmicronOptions options) {
		...
		System.out.println("Initializing GdxOmicron");
		this.hw = hw;
	...
	private void initOrResumeGameRun(GameRun r) {
		...
		System.out.println("appType: "+Gdx.app.getType());
...
//			}
	...
	{
		if(Gdx.app.getType() == ApplicationType.WebGL) // using TeaVM
		{
			FileHandle ff = Gdx.files.internal("omicrondefaultfont.png");
			try(InputStream is = ff.read())
			{
