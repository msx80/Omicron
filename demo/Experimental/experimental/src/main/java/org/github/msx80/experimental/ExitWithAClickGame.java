...
public class ExitWithAClickGame implements Game, CustomResourceGame {

	private Game src;
	private Sys sys;
	
	
	public ExitWithAClickGame(Game src) {
		this.src = src;
	}
	...
	public boolean update() {
		if(sys.mouse().btn[0])
		{
			((AdvancedSys)sys).quit("Closing with a click");
			return false;
		} else	return src.update();
	}
